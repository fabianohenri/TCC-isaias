...
const Login = ({ loginActionDispatch }) => {
  ...
  useEffect(() => {
    ...
    if (code && scope) {
      setLoading(true);
      api.get(`/login?authCode=${code}&scope=${scope}&domain=${domain}`).then((res) => {
        setLoading(false);
        loginActionDispatch(res.data);
      });
    }
  }, [searchParams, loginActionDispatch]);
  ...
  return (
    ...
    >
      ...
      >
        <CardHeader
          title="Login"
          subheader="Faça o login utilizando o seu Bitrix24"
        />
        <Divider />
        <Grid container style={{ marginTop: '5em', marginBottom: 'auto' }}>
          ...
          <Grid item xs={2}>
            <Button
              ...
              disabled={loading}
              variant="contained"
              style={{ height: '100%', marginLeft: '3em' }}
            >
        ...
        <Grid container style={{ marginBottom: 'auto', marginTop: '0.3em' }}>
          <Grid item xs>
            <Typography
              style={{ marginLeft: '0.2em', opacity: '0.8' }}
